...
import { VehicleTagForm } from '@/components/forms/VehicleTagForm';
...
export default function VehicleTagsPage() {
  const t = useTranslations('vehicleTags');
  const tc = useTranslations('common');
  const { can } = usePermissions();

  const [data, setData] = useState<VehicleTag[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedTag, setSelectedTag] = useState<VehicleTag | null>(null);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const tags = await vehicleTagsService.getAll();
      setData(tags);
    } catch (error) {
      console.error('Failed to fetch vehicle tags:', error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleView = (tag: VehicleTag) => {
    setEditingTag(tag);
    setFormOpen(true);
  };
  ...
  const handleDelete = async (tag: VehicleTag) => {
    ...
    setDeleteError(null);
    try {
      await vehicleTagsService.delete(tag.id);
      fetchData();
    } catch (error) {
      console.error('Failed to delete vehicle tag:', error);
  ...
  const handleSubmit = async (data: VehicleTagCreateRequest | VehicleTagUpdateRequest) => {
    try {
      if (editingTag) {
        await vehicleTagsService.update(editingTag.id, data);
      } else {
        await vehicleTagsService.create(data);
      }
      fetchData();
    } catch (error) {
      console.error('Failed to save vehicle tag:', error);
    }
  };

  const columns: ColumnDef<VehicleTag>[] = [
  ...
  const renderActions = (row: VehicleTag): ActionItem[] => [
    can('vehicles-tags.update')
      ? { icon: Edit, label: tc('edit'), onClick: () => handleEdit(row) }
      : { icon: Eye, label: tc('view'), onClick: () => handleView(row) },
    ...(can('vehicles-tags.delete') ? [{ icon: Trash2, label: tc('delete'), onClick: () => handleDelete(row), destructive: true }] : []),
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <VehicleTagForm
        ...
        readOnly={!can('vehicles-tags.update')}
      />
    </div>
  );
}
