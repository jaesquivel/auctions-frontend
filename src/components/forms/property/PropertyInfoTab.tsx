...
import { formatCurrency, formatRatio, formatDate, formatArea } from '@/lib/formatters';
...
export function PropertyInfoTab({ property, formData, setFormData, readOnly }: PropertyInfoTabProps) {
  ...
  const moneyDisplay = (value: string, currency?: string) => (
    <div className="flex h-9 w-full max-w-[200px] items-center rounded-md border border-input bg-transparent px-3 py-1 text-sm text-right text-muted-foreground">
      ...
      {property && (
        <div className="rounded-md border p-3 bg-muted/50 space-y-1 text-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-1 text-muted-foreground">
            <p className="text-right"><span className="font-medium">{t('fiscalValueUsd')}:</span> {formatCurrency(property.fiscalValueUsd, 'USD')}</p>
            <p className="text-right"><span className="font-medium">{t('firstBaseAdj')}:</span> {formatCurrency(property.firstAuctionBaseAdj, cur)}</p>
            <p className="text-right"><span className="font-medium">{t('firstGuarantee')}:</span> {formatCurrency(property.firstAuctionGuarantee, cur)}</p>
